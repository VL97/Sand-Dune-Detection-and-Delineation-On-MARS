'''
THIS CODE TRAINS THE MODEL ON 'INPUTS' AND 'MASKS' FROM 'TRAIN' FOLDER AND 
VALIDATES IT ON DIFFERENT 'INPUTS' AND 'MASKS' ON THE 'VALID' FOLDER. THE CODE 
SAVES THE COMPLETE MODEL AFTER EVERY 2 EPOCHS AND RUNS FOR TOTAL 5 EPOCHS. THE FINAL 
MODEL IS ALSO SAVED AFTER THE LAST EPOCH.
IMPORTANT: 
CORRESPONDING INPUT IMAGES AND MASK IMAGES IN THE TRAIN FOLDER MUST 
HAVE THE SAME NAME! 
'''
...
for i in os.listdir('train/inputs'):
    if '.png' in i:
...
for i in os.listdir('valid/inputs'):
    if '.png' in i:
...
path_to_X_train='train/inputs/'
path_to_Y_train='train/masks/'

path_to_X_valid='valid/inputs/'
path_to_Y_valid='valid/masks/'
...
checkpoint = ModelCheckpoint(filepath, monitor='val_dice_coef', verbose=1, save_best_only=False,\
...
model.save('final_model_saved.h5')

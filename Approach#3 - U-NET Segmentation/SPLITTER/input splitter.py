...
for i in range(density_passes_made_x):
    ...
    for j in range(density_passes_made_y):
        ...
        cv2.imwrite("LBP INPUTS FOR TRAINING/"+str(i)+str(j)+"a_orig.png",image_sliced)
        cv2.imwrite("LBP INPUTS FOR TRAINING/"+str(i)+str(j)+"b_rotated.png",dst)
        cv2.imwrite("LBP INPUTS FOR TRAINING/"+str(i)+str(j)+"c_horiflip.png",flipped1)
        cv2.imwrite("LBP INPUTS FOR TRAINING/"+str(i)+str(j)+"d_vertiflip.png",flipped0)
